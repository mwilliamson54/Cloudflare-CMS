...
export const onRequestGet = async ({ env, request }: Context) => {
  ...
  const entries = await env.CMS_DB.prepare("SELECT e.slug,e.updated_at,t.key FROM content_entries e JOIN content_types t ON t.id=e.content_type_id WHERE e.status='published' AND e.robots_index=1 ORDER BY e.updated_at DESC LIMIT 49999").all();
