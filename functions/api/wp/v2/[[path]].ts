...
async function verifyJwt(token: string, secret: string) {
  ...
  if (!["admin", "editor", "author", "contributor", "subscriber", "viewer"].includes(String(claims.role))) throw new Error("Unsupported CMS role.");
  return claims as { sub: string; jti: string; role: "admin" | "editor" | "author" | "contributor" | "subscriber" | "viewer"; scopes: string[] };
}
...
async function resourceList(context: Context, resource: string) {
  ...
  if (resource === "media") { const count = await context.env.CMS_DB.prepare("SELECT COUNT(*) AS total FROM media").first(); const rows = await context.env.CMS_DB.prepare("SELECT * FROM media ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(perPage, (page - 1) * perPage).all(); const total = Number(count?.total || 0); return json(rows.results, 200, { "X-WP-Total": String(total), "X-WP-TotalPages": String(Math.max(1, Math.ceil(total / perPage))) }); }
  if (resource === "users") { const count = await context.env.CMS_DB.prepare("SELECT COUNT(*) AS total FROM users").first(); const rows = await context.env.CMS_DB.prepare("SELECT id,name,created_at FROM users ORDER BY id LIMIT ? OFFSET ?").bind(perPage, (page - 1) * perPage).all(); const total = Number(count?.total || 0); return json(rows.results.map((user: any) => ({ id: user.id, name: user.name || "CMS author", slug: `author-${user.id}`, link: `/author/${user.id}`, description: "", avatar_urls: {}, registered_date: user.created_at })), 200, { "X-WP-Total": String(total), "X-WP-TotalPages": String(Math.max(1, Math.ceil(total / perPage))) }); }
...
export const onRequestPost = async (context: Context) => {
  ...
  try {
    if (resource === "posts" || resource === "pages") { const claims = await requireToken(context, "content:write"); const status = ["draft","scheduled","published","archived"].includes(input.status) ? input.status : "draft"; if ((status === "published" || status === "scheduled") && !["admin", "editor", "author"].includes(claims.role)) return wpError(403, "cms_forbidden", "This API token does not grant publishing access."); const type = resource === "posts" ? "post" : "page"; const typeRow = await context.env.CMS_DB.prepare("SELECT id FROM content_types WHERE key=?").bind(type).first(); if (!typeRow || !input.title || !input.slug) return wpError(400, "rest_invalid_param", "title and slug are required."); const now = new Date().toISOString(); const result = await context.env.CMS_DB.prepare("INSERT INTO content_entries (content_type_id,author_id,title,slug,excerpt,body_markdown,status,scheduled_at,published_at,seo_title,seo_description,canonical_url,robots_index,robots_follow,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(typeRow.id, Number(claims.sub), String(input.title.raw || input.title), String(input.slug), input.excerpt?.raw || input.excerpt || null, input.content?.raw || input.content || "", status, status === "scheduled" ? input.date : null, status === "published" ? now : null, input.meta?.seo_title || null, input.meta?.seo_description || null, input.meta?.canonical_url || null, input.meta?.robots_index === false ? 0 : 1, input.meta?.robots_follow === false ? 0 : 1, now, now).run(); return json({ id: result.meta.last_row_id, status, slug: input.slug, title: { rendered: input.title.raw || input.title } }, 201); }
    if (resource === "media") { const claims = await requireToken(context, "media:write"); if (!input.fileName || !input.mimeType || !input.dataBase64) return wpError(400, "rest_upload_invalid", "fileName, mimeType, and dataBase64 are required."); const supported = ["image/jpeg", "image/png", "image/webp", "image/avif", "image/gif", "application/pdf"]; if (!supported.includes(input.mimeType)) return wpError(400, "rest_upload_invalid_type", "Unsupported media type."); const bytes = fromBase64Url(String(input.dataBase64).replace(/\+/g, "-").replace(/\//g, "_")); if (!bytes.byteLength || bytes.byteLength > 10 * 1024 * 1024) return wpError(413, "rest_upload_too_large", "Media uploads must be between 1 byte and 10 MB."); const original = String(input.fileName).slice(0,255); const safe = original.replace(/\.[^.]+$/, "").replace(/[^a-zA-Z0-9_-]/g, "-").slice(0,160) || "upload"; const ext = ({ "image/jpeg":"jpg", "image/png":"png", "image/webp":"webp", "image/avif":"avif", "image/gif":"gif", "application/pdf":"pdf" } as Record<string,string>)[input.mimeType]; const date = new Date(); const key = `uploads/${date.getUTCFullYear()}/${String(date.getUTCMonth()+1).padStart(2,"0")}/u${claims.sub}/${safe}-${crypto.randomUUID().slice(0,8)}.${ext}`; await context.env.CMS_MEDIA.put(key, bytes, { httpMetadata: { contentType: input.mimeType } }); const url = `/media/${key.split("/").map(encodeURIComponent).join("/")}`; const result = await context.env.CMS_DB.prepare("INSERT INTO media (storage_key,storage_provider,url,file_name,original_file_name,mime_type,size_bytes,alt_text,title,uploaded_by_id) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(key,"cloudflare-r2",url,`${safe}.${ext}`,original,input.mimeType,bytes.byteLength,input.alt_text || null,input.title || null,Number(claims.sub)).run(); return json({ id: result.meta.last_row_id, source_url: url, alt_text: input.alt_text || "" },201); }
    return wpError(404,"rest_no_route","No route was found matching the URL and request method.");
