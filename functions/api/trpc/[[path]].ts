...
export const appRouter = t.router({
  ...
  cms: t.router({
    ...
    menus: t.router({
      list: protectedProcedure.query(async ({ ctx }) => { const rows = (await ctx.env.CMS_DB.prepare("SELECT id,name,location,items,created_at,updated_at FROM menus ORDER BY location,name").all<Record<string, unknown>>()).results; return rows.map(row => ({ ...row, id: Number(row.id), items: (() => { try { return JSON.parse(String(row.items)); } catch { return []; } })() })); }),
      save: mutation.input(z.object({ name: z.string().min(1).max(120), location: z.string().min(1).max(80), items: z.array(z.object({ id: z.string().min(1).max(120), label: z.string().min(1).max(120), target: z.enum(["page", "post", "category", "url"]), targetId: z.number().int().positive().optional(), url: z.string().max(2000).optional(), children: z.array(z.any()).max(20).optional() })).max(30) })).mutation(async ({ ctx, input }) => { requireRole(ctx.session.user.role, ["admin"]); const now = new Date().toISOString(); await ctx.env.CMS_DB.prepare("INSERT INTO menus (name,location,items,created_at,updated_at) VALUES (?,?,?,?,?) ON CONFLICT(location) DO UPDATE SET name=excluded.name,items=excluded.items,updated_at=excluded.updated_at").bind(input.name, input.location, JSON.stringify(input.items), now, now).run(); const row = await ctx.env.CMS_DB.prepare("SELECT id,name,location,items,created_at,updated_at FROM menus WHERE location=?").bind(input.location).first<Record<string, unknown>>(); return row ? { ...row, id: Number(row.id), items: input.items } : null; }),
    }),
    media: t.router({
    ...
    users: t.router({
      list: protectedProcedure.query(async ({ ctx }) => { requireRole(ctx.session.user.role, ["admin"]); const rows = (await ctx.env.CMS_DB.prepare("SELECT id,email,name,role,created_at,updated_at FROM users ORDER BY id").all<Record<string, unknown>>()).results; return rows.map(row => ({ ...row, id: Number(row.id), role: String(row.role) })); }),
      updateRole: mutation.input(z.object({ id: z.number().int().positive(), role: z.enum(["admin", "editor", "author", "contributor", "subscriber", "viewer"]) })).mutation(async ({ ctx, input }) => { requireRole(ctx.session.user.role, ["admin"]); if (input.id === ctx.session.user.id && input.role !== "admin") throw new TRPCError({ code: "FORBIDDEN", message: "Administrators cannot demote themselves." }); const result = await ctx.env.CMS_DB.prepare("UPDATE users SET role=?,updated_at=? WHERE id=?").bind(input.role, new Date().toISOString(), input.id).run(); return { id: input.id, role: input.role, updated: Number(result.meta.changes ?? 0) > 0 }; }),
    }),
    apiTokens: t.router({
      list: protectedProcedure.query(async ({ ctx }) => { requireRole(ctx.session.user.role, ["admin", "editor", "author", "contributor"]); const rows = (await ctx.env.CMS_DB.prepare("SELECT id,name,token_prefix,scopes,expires_at,last_used_at,revoked_at,created_at FROM api_tokens WHERE user_id=? ORDER BY created_at DESC").bind(ctx.session.user.id).all<Record<string, unknown>>()).results; return rows.map(row => ({ ...row, id: Number(row.id), scopes: (() => { try { return JSON.parse(String(row.scopes)); } catch { return []; } })() })); }),
      revoke: mutation.input(z.object({ id: z.number().int().positive() })).mutation(async ({ ctx, input }) => { requireRole(ctx.session.user.role, ["admin", "editor", "author", "contributor"]); await ctx.env.CMS_DB.prepare("UPDATE api_tokens SET revoked_at=? WHERE id=? AND user_id=?").bind(new Date().toISOString(), input.id, ctx.session.user.id).run(); return { success: true }; }),
      create: mutation.input(z.object({ name: z.string().min(1).max(120), scopes: z.array(z.enum(["content:read", "content:write", "media:write", "taxonomy:write"])).min(1), expiresInDays: z.number().int().min(1).max(365).default(30) })).mutation(() => unsupported()),
    }),
    themes: t.router({
      list: protectedProcedure.query(async ({ ctx }) => { requireRole(ctx.session.user.role, ["admin"]); return (await ctx.env.CMS_DB.prepare("SELECT * FROM themes ORDER BY name").all<Record<string, unknown>>()).results; }),
    }),
    plugins: t.router({
      list: protectedProcedure.query(async ({ ctx }) => { requireRole(ctx.session.user.role, ["admin"]); return (await ctx.env.CMS_DB.prepare("SELECT * FROM plugins ORDER BY name").all<Record<string, unknown>>()).results; }),
      setActivation: mutation.input(z.object({ key: z.string().min(1).max(120), active: z.boolean() })).mutation(async ({ ctx, input }) => { requireRole(ctx.session.user.role, ["admin"]); await ctx.env.CMS_DB.prepare("UPDATE plugins SET is_active=? WHERE key=?").bind(input.active ? 1 : 0, input.key).run(); return { key: input.key, active: input.active }; }),
    }),
    settings: t.router({
