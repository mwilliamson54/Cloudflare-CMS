...
export function FashionHeader() {
  ...
  const navigation = headerMenu?.items.length ? headerMenu.items.map(item => ({ label: item.label, href: item.url || "/" })) : fashionTheme.navigation;
