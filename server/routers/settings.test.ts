...
function adminContext(): TrpcContext {
  return {
    user: { id: 1, openId: "admin-1", name: "Administrator", email: "admin@example.com", loginMethod: "test", role: "admin", createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() },
...
describe("CMS controlled custom CSS settings", () => {
  ...
  it("persists accepted local CSS as a public setting", async () => {
    await cmsRouter.createCaller(adminContext()).settings.update({ ...settingsInput, customCss: ".site-accent { color: #a77150; }" });
  ...
  it("rejects remote imports and does not persist unsafe CSS", async () => {
    await expect(cmsRouter.createCaller(adminContext()).settings.update({ ...settingsInput, customCss: '@import url("https://example.test/theme.css");' })).rejects.toMatchObject({ code: "BAD_REQUEST" });
