...
The protected tRPC administration path now applies an explicit same-origin guard before routing a request into the CMS API. Browser-supplied `Origin` headers must match the receiving host; malformed or cross-origin requests receive a `403` response. Requests with no Origin header remain available for non-browser operations, while the WordPress-compatible REST API retains its separate bearer-token authentication and scope model. No custom head/body/CSS/JS execution settings are exposed in the current administration UI; this is intentional until a separately reviewed trust model and CSP strategy are implemented.
...
The origin guard is regression-tested for same-origin browser requests, origin-less non-browser requests, malformed values, and hostile cross-origin values. It provides an explicit baseline for the current single-origin administration deployment; any future intentionally cross-origin CMS integration must use a reviewed CSRF token or equivalent request-authentication protocol rather than bypassing this guard.
